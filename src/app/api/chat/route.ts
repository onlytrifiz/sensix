...
export async function POST(req: Request) {
  ...
  console.log('[chat/route] POST called');
  ...
  console.log('[chat/route] session', { userId, publicKey, degenMode });

  if (!userId) {
    console.error('[chat/route] Unauthorized: no userId');
  ...
  try {
    ...
    const { id: conversationId, message }: { id: string; message: Message } =
      await req.json();
    console.log('[chat/route] received', { conversationId, message });
    ...
    console.log('[chat/route] existingMessages', existingMessages);
    logWithTiming(startTime, '[chat/route] fetched existing messages');

    if (existingMessages.length === 0 && message.role !== 'user') {
      console.error('[chat/route] No user message found in empty conversation');
